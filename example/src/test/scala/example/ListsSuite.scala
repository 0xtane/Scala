 ...
 */
 class ListsSuite {
  ...
  @Test def `one plus one is three (0pts)?`: Unit = {
    Assert.assertEquals(2, 1 + 1)// This assertion fails! Go ahead and fix it.
  }
  ...
  @Test def `sum of a few numbers (10pts)`: Unit = {
    ...
    assert(sum(List(1,2,100,2,435,666666)) == 667206)
  }

  @Test def `max of a few numbers (10pts)`: Unit = {
    assert(max(List(3, 7, 2)) == 7)
    assert(max(List(3, 71212, 2122 , 71212 ,99999,0)) == 99999)
  }
